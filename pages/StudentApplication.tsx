...
import { Flex, Text, Select, Box, Grid, Card, CardBody, Button, useToast, FormControl, FormLabel, Checkbox} from '@chakra-ui/react';
...
export default function StudentApplication({schoolYears}:SchoolYearInterface) {
                ...
                <CardBody p={'0pc'} pb={'5pc'}>
                  ...
                  <Grid templateColumns={['repeat(1, 1fr)','repeat(2, 1fr)','repeat(3, 1fr)','repeat(4, 1fr)']} gap={'3'} p={'2pc'}>
                    <FormControl isInvalid={formik.errors.school_year && formik.touched.school_year}>
                      ...
                      <FastField as={Select} size={'sm'} name='school_year' backgroundColor={'white'} placeholder=' '>
                        {
                          schoolYears.map(item=>{
                            return <option key={item.school_yr_id} value={item.school_yr_id}>{new Date(item.start_date).getFullYear() + '-' + new Date(item.end_date).getFullYear()}</option>
                          })
                        }
                      </FastField>
                    </FormControl>
                    <FormControl isInvalid={formik.errors.year_level && formik.touched.year_level}>
                      <FormLabel fontSize={'13px'} color={'blue.800'} fontWeight={'regular'}>2. Grade level</FormLabel>
                      <FastField as={Select} size={'sm'} name='year_level' backgroundColor={'white'} placeholder=' '>
                        {
                          yearlevels.map(item=>{
                            return <option key={item} value={item}>{'Grade ' + item}</option>
                          })
                        }
                      </FastField>
                    </FormControl>
                  ...
                  <Flex w={'full'} pl={'2pc'} pr={'2pc'} pb={'1pc'} flexDirection={'column'}>
                    <Flex gap={'5'}>
                      ...
                      <Checkbox size={'sm'} color={'blue.700'} onChange={()=>{setIsSameWithCurAddress(!isSameWithCurAddress)}}>Same with Current Address?</Checkbox>                    
                    </Flex>
                    <Box w={'full'} borderBottomColor={'gray.200'} borderBottomWidth={'1px'}></Box>
                  </Flex>
                  {
                    !isSameWithCurAddress? (
                      <Grid templateColumns={['repeat(1, 1fr)','repeat(2, 1fr)','repeat(3, 1fr)','repeat(4, 1fr)']} gap={'3'} pr={'2pc'} pl={'2pc'} pb={'2pc'}>
                  ...
                  <Grid templateColumns={['repeat(1, 1fr)','repeat(1, 1fr)','repeat(2, 1fr)','repeat(2, 1fr)']} p={'2pc'} gap={'4'}>
                    ...
                    <Button type="button" color={'gray.500'} onClick={()=>{router.push('/SignIn')}}>Cancel</Button>
                  </Grid>
                </CardBody>
              </form>
          )}
